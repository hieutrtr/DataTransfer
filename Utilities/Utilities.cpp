...
string CUtilities::FormatDateSuffixLogByDay(struct tm tm)
{
	stringstream ssDatetimeSuffix;
	string strSuffix;
...
vector<string> CUtilities::GetListZabbixProcessId(string strPathPattern, string strDatetime)
{
	...
	string strFileName;
	int iProcessId;
	if ((dp = opendir(strPathPattern.c_str())) == NULL)
	{
		return vtFileName;
	}

	while ((stDirp = readdir(dp)) != NULL) {
		strFileName = string(stDirp->d_name);
		if (strFileName.find(strDatetime) != std::string::npos)
		{
			vtFileName.push_back(strFileName);
		}
	}
	closedir(dp);
	return vtFileName;
...
string CUtilities::GetDateSuffixLogByDay()
{
	string strSuffix;

	time_t t = time(NULL);
	struct tm tm = *localtime(&t);
	strSuffix = FormatDateSuffixLogByDay(tm);
...
IP_TYPE CUtilities::GetIPType(string strIP)
{
